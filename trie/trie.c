...
/**
 * A program which searches a word in a trie.
*/
...
bool check(char* word)
{
    // begin from the root, check if address at particular index is NULL or Not
    // if NULL return false
    // Else move temp to that index and repeat until the end of the word at which point check if the is_word value is true.
    node *temp = root;
    for (int j = 0; word[j] != '\0'; j++)
    {
        for (int i = 0; i < SIZE_OF_ALPHABET; i++)
        {
            if (i == (tolower(word[j]) - 'a') && temp->children[i] != NULL)
            {
                temp = temp->children[i];
                break;
            }
            else if (i == (tolower(word[j]) - 'a') && temp->children[i] == NULL)
            {
                return false;
            }
        }
    ...
    // check if the last letter is the end of a word.
